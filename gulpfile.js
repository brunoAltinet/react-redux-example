...
var CORDOVA_DEVELOPER_APP_PORT = configs.cordovaServePort
var destW=isCordova?(CORDOVA_APP_DIR + '/www/'):build;
...
gulp.task('html', function () {
    return gulp.src(isCordova ? app + 'index-cordova.html': app + 'index.html')
        ...
        .pipe(gulp.dest(destW))
});

// Images
gulp.task('images', function () {
    return gulp.src(app+'images/**')
        .pipe(gulp.dest(destW + 'images'));
});


gulp.task('cordova-serve', shell.task([
    'cordova serve --port=' + CORDOVA_DEVELOPER_APP_PORT
], {
    cwd: CORDOVA_APP_DIR
}));
...
gulp.task('run', function () {
    isCordova ?
        gulp.start(['webpack-build','images', 'html', 'webpack-watch', 'cordova-serve']) :
        gulp.start(['webpack-build','images', 'html', 'webpack-dev-server']);
});


/* Creating the cordova app. Creates the right project dirs and install plugins
 * source: https://github.com/kjda/ReactJs-cordova
 */

gulp.task('cordova-create', function (cb) {
    runSequence('clean-app', 'create-app',
        //'install-plugins',
        cb);
});
...
gulp.task('create-app', shell.task([
    'cordova create ' + CORDOVA_APP_DIR
]));
...
function getPhonegapPluginCommands() {
    var commands = [];
    for (var i = 0; i < configs.app.cordovaPlugins.length; i++) {
        var p = configs.app.cordovaPlugins[i];
        commands.push('cordova plugin add ' + p.name);
    }
    return commands;
}
